...
split_half_testing <- function(covariates, Y, treatment, no_repeats = 10, n_core = 8){
    ...
    correlation_matrix <- foreach(i = seq(no_repeats), .combine = 'rbind') %dopar%  {
        ...
        # params = tune_causal_forest(X.covariates[trainId, ], Y[trainId], treatment[trainId])$params
        tau.forest.train <- cf.tuned(covariates[trainId, ], Y[trainId], treatment[trainId])   # run causal forests by default
        ...
        tau.forest.test <- cf.tuned(covariates[-trainId, ], Y[-trainId], treatment[-trainId])   # run causal forests by default
...
run.hte <- function(mat, mutations, dataset, trainId, is.binary = TRUE, thres = 0.75, n_core = 8){
    ...
    for(mut in mutations){
        ...
        aggregated_ret <- split_half_testing(X.covariates, Y, treatment)
        ...
        # *****************************************************************************************
        tau.forest <- cf.tuned(X.covariates, Y, treatment)   # run causal forests by default
        ...
        if(simes.pval <= 0.05){ 
            ...
            write.csv(pred.ret, paste0('mutation_result/', cancerType, '/', cancerType, '.tau.', mut, '.csv'), quote = F, row.names = F)
            ...
            write.csv(varImp.ret, paste0('mutation_result/', cancerType, '/', cancerType, '.varimp.', mut, '.csv'), quote = F, row.names = F) 
        }
    }
...
write.csv(result[[4]], paste0('mutation_result/', cancerType, '/', cancerType, '.mutation.permutate.testing.result.csv'), quote = F, row.names = F)
